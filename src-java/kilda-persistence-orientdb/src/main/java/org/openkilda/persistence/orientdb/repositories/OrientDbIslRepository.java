...
public class OrientDbIslRepository extends FermaIslRepository {
    ...
    private static final String ISL_VIEW_FIELDS = String.join(", ",
            ...
            IslFrame.MAX_BANDWIDTH_PROPERTY, IslFrame.AVAILABLE_BANDWIDTH_PROPERTY,
            IslFrame.STATUS_PROPERTY, IslFrame.UNDER_MAINTENANCE_PROPERTY,
    ...
    public Collection<IslView> findActiveByPathAndBandwidthAndEncapsulationType(
        ...
        List<IslView> isls = new ArrayList<>(segments.size());
    ...
    public Collection<IslView> findAllActive() {
        ...
        try (OGremlinResultSet results = orientDbGraphFactory.getOrientGraph().querySql(
            ...
            results.forEach(gs -> {
                ...
                if (switches.containsKey(srcSwitch) && switches.containsKey(dstSwitch)) {
                    isls.add(mapToIslView(gs, switches.get(srcSwitch), switches.get(dstSwitch)));
                }
            });
        }
        return isls;
    ...
    protected Map<IslEndpoints, IslView> findActiveByAvailableBandwidthAndEncapsulationType(
        ...
        Map<IslEndpoints, IslView> isls = new HashMap<>();
        try (OGremlinResultSet results = orientDbGraphFactory.getOrientGraph().querySql(
            ...
            results.forEach(gs -> {
                ...
                if (switches.containsKey(srcSwitch) && switches.containsKey(dstSwitch)) {
                    ...
                    isls.put(key, mapToIslView(gs, switches.get(srcSwitch), switches.get(dstSwitch)));
                }
            });
        }
        return isls;
    ...
    private IslView mapToIslView(OGremlinResult gs, String srcPop, String dstPop) {
        return new IslViewImpl(
                ...
                gs.getProperty(IslFrame.MAX_BANDWIDTH_PROPERTY),
                gs.getProperty(IslFrame.AVAILABLE_BANDWIDTH_PROPERTY),
                IslStatusConverter.INSTANCE.toEntityAttribute(gs.getProperty(IslFrame.STATUS_PROPERTY)),
