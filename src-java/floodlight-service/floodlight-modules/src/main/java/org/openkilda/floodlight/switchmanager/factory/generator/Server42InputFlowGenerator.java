...
public class Server42InputFlowGenerator implements SwitchFlowGenerator {
    ...
    public SwitchFlowTuple generateFlow(IOFSwitch sw) {
        ...
        OFFlowMod flowMod = prepareFlowModBuilder(
                ofFactory, SERVER_42_INPUT_COOKIE, ROUND_TRIP_LATENCY_RULE_PRIORITY, INPUT_TABLE_ID)
                .setMatch(match)
                .setActions(actions)
                .build();
    ...
    private Match buildMatch(DatapathId dpid, OFFactory ofFactory) {
        return ofFactory.buildMatch()
                ...
                .setExact(MatchField.UDP_SRC, TransportPort.of(4700))
                .setExact(MatchField.UDP_DST, TransportPort.of(4701))
                .build();
    }
