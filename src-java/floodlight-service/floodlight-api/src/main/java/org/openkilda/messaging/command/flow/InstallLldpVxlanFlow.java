...
public class InstallLldpVxlanFlow extends ConnectedDeviceVxlanFlowBase {
    private static final long serialVersionUID = 4099967122813811679L;

    @JsonCreator
    public InstallLldpVxlanFlow(@JsonProperty(TRANSACTION_ID) final UUID transactionId,
        ...
        super(transactionId, id, cookie, switchId, inputPort, outputPort, inputVlanId, transitEncapsulationId, meterId,
                egressSwitchId);
        if (!Cookie.isMaskedAsLldpVxlan(cookie)) {
            throw new IllegalArgumentException(format("Invalid cookie %s for InstallLldpVxlanFlow command", cookie));
        }
    }
}
